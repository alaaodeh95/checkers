...
export const getMoves = (cords: [number, number], player: Player) => (board: Board) => {
    ...
    return {
        ...
        isKiller,
        mobility: [...possibleKillerMoves, possibleMoves]
    };
};
...
export const exploreMove = (from: [number, number], to: [number, number], board: Board, currentPlayer: Player, moves: AIMove[] = []): { board: Board; moves: AIMove[] } => {
    ...
    // Empty from square
    //const locationHistory = [...clone(board[from[0]][from[1]].piece.locationHistory), to];
    board[from[0]][from[1]] = { location: from, piece: { id: PieceId.Null } };
    ...
    if (shouldKill) {
        ...
        board[deadX][deadY] = { location: from, piece: { id: PieceId.Null } };
    }
    ...
    board[to[0]][to[1]] = { location: to, piece: { id: newPieceType } };
    moves.push({ from, to });
